...
export function AppointmentDetails() {
    ...
    function fetchGuildWidget() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield api.get(`/guilds/${guildSelected.guild.id}/widget.json`);
    ...
    return (<Background>
      ...
      {loading ? <Load /> :
          ...
          <FlatList data={widget.members} keyExtractor={item => item.id} renderItem={({ item }) => (<Member data={item}/>)} ItemSeparatorComponent={() => <ListDivider isCentered/>} style={styles.members}/>          
        </>}
